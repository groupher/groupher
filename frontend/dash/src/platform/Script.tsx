...
const PLATFORM_LOADED = 'data-platform-loaded'
...
/**
 * Provide Next Script-compatible loading semantics for the TanStack application.
 */
export default function NativePlatformScript({
  ...
  useEffect(() => {
    ...
    const handleLoad = (event: Event) => {
      ...
      script.setAttribute(PLATFORM_LOADED, '')
    ...
    const insert = () => {
      ...
      if (existing) {
        ...
        if (existing.hasAttribute(PLATFORM_LOADED)) {
          queueMicrotask(() => {
      ...
      if (dangerouslySetInnerHTML?.__html) {
        script.text = String(dangerouslySetInnerHTML.__html)
      }
    ...
    return () => {
      ...
      if (ownsScript && !scriptElement?.hasAttribute(PLATFORM_LOADED)) {
