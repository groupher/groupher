import { COLOR } from '~/const/colors'

import useTheme from '~/hooks/useTheme'
import useTwBelt from '~/hooks/useTwBelt'

export { cn } from '~/css'

export default () => {
  const { isLightTheme } = useTheme()
  const { cn, rainbow, fill } = useTwBelt()

  const turnning = isLightTheme ? 'opacity-30 saturate-50' : 'opacity-15 saturate-100'

  return {
    wrapper: 'absolute s-full top-0 left-0',
    wipItem: cn('row-center absolute left-1/2 top-28 -ml-32 rotate-3', rainbow(COLOR.GREEN, 'fg')),
    wipText: cn('text-lg bold opacity-30 ml-1', rainbow(COLOR.GREEN, 'fg')),
    wipIcon: cn('size-4 opacity-50 -mt-0.5', rainbow(COLOR.GREEN, 'fill')),

    versionText: cn(
      'absolute top-28 right-72 text-base bold opacity-40 -rotate-2',
      rainbow(COLOR.GREEN, 'fg'),
    ),

    thxText: cn(
      'absolute bottom-36 left-1/2 -ml-32 text-base bold opacity-25 rotate-2',
      rainbow(COLOR.GREEN, 'fg'),
    ),

    topicItem: cn('row-center absolute left-20 bottom-32 -rotate-3', rainbow(COLOR.GREEN, 'fg')),

    waweLine: cn(
      'size-10 absolute -bottom-8 right-0 rotate-12 opacity-20 scale-y-50',
      rainbow(COLOR.ORANGE, 'fill'),
    ),

    curveLineTL: cn('absolute size-80 top-24 left-60', turnning),
    curveLineBL: cn('absolute size-80 bottom-20 left-60 ml-2.5 mb-1', turnning),
    //
    curveLineTR: cn('absolute size-80 top-24 right-48', turnning),
    curveLineBR: cn('absolute size-80 bottom-20 right-44 mb-1', turnning),
    fillPurple: rainbow(COLOR.PURPLE, 'fill'),
    fillDigest: fill('digest'),
    //
    shapeIcon: 'size-5 absolute opacity-30',
    fillGreen: rainbow(COLOR.GREEN, 'fill'),
    fillRed: rainbow(COLOR.RED, 'fill'),
    fillOrange: rainbow(COLOR.ORANGE, 'fill'),
    fillBlue: rainbow(COLOR.BLUE, 'fill'),
    squareIcon: cn('size-4 absolute rounded rotate-45 opacity-20', rainbow(COLOR.GREEN, 'bg')),
    circleIcon: cn('size-4 circle border-4 absolute opacity-10', rainbow(COLOR.GREEN, 'border')),
  }
}
