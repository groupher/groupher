...
export default () => {
  ...
  return {
    wrapper: 'column-align-both size-20 group/position',
    block: cn(
    ...
    pice: cn(
      'size-3.5 circle border pointer trans-all-100',
      br('divider'),
      bg('hoverBg'),
      `hover:${primary('border')}`,
    ),
    piceActive: cn(primary('bg')),
    title: cn('text-xs scale-75 -mt-3.5 group-hover/position:hidden', fg('digest')),
  }
}
