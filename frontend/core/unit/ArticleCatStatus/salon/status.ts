...
export default function useSalon({ cat, status }: TProps) {
  const { cn, rainbow, rainbowLite } = useTwBelt()
  ...
  return {
    box: cn('align-both size-4 mr-0.5 rounded', rainbowLite(statusColor[status] || COLOR.RED)),
    text: 'text-xs py-1.5',
    ...
    rejectIcon: cn('size-3 z-10', rainbow(COLOR.RED, 'fill')),
  }
}
