...
export default function useSalon() {
  ...
  return {
    ...
    textTrackItem: 'block',
    publishGroup: 'row-center ml-1 shrink-0',
    publishButton: cn(
    ...
    publishMenu: cn('column min-w-80 p-2 rounded-lg gap-2', fg('digest')),
    publishMenuSection: 'column gap-1',
    publishMenuHeading: cn('row-center gap-1 px-2 pb-1 text-xs bold-sm', fg('title')),
    publishMenuCount: cn('text-xs', fg('digest')),
    publishMenuEmpty: cn('px-2 py-1 text-xs', fg('digest')),
    publishMenuItem: cn(
      'grid grid-cols-[18px_minmax(0,1fr)] gap-2 w-full rounded-md px-2 py-2 text-left cursor-pointer',
      hover('box'),
    ),
    publishMenuCheckbox: cn('mt-0.5 size-3.5 accent-current', primary('fg')),
    publishMenuText: 'min-w-0',
