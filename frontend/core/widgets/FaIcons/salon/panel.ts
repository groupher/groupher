...
export default () => {
  const { cn, bg, hoverable, rainbow } = useTwBelt()

  return {
    wrapper: cn('w-36 py-0.5'),
    colorWrapper: 'align-both mt-2 pb-3.5 gap-x-2.5 gap-y-1.5',
    item: cn('row-center pl-2.5 px-1.5 py-1 last:mb-2.5', hoverable('bg')),
    itemActive: cn(bg('hoverBg')),
    iconBox: cn('align-both size-4 -mt-0.5 opacity-80', hoverable('icon')),
    title: cn('text-xs ml-2.5', hoverable('fg')),
