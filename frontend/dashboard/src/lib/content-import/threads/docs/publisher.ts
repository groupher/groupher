...
export const runPreviewDocBulkImport = async (
  ...
  let completed = 0
  let latestStage: TDocImportBodyStageResult | null = null

  const flush = async (): Promise<void> => {
    if (batch.length === 0) return
    latestStage = await stageDocImportBodies(community, jobRef, batch, options)
  ...
  if (latestStage?.status.toLowerCase() === 'failed') return latestStage
