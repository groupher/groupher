defmodule GroupherServer.CMS.Model.DocPublishReleaseTreeEvent do
  @moduledoc """
  Tree event membership for one docs release.

  `DocPublishRelease.tree_snapshot_id -> DocTreeSnapshot.tree_json` is the complete
  ...
  event-level revert.
      ...
      doc_publish_releases.tree_snapshot_id
           |  full state snapshot
           v
  ...
  @type t :: %DocPublishReleaseTreeEvent{}
  schema "doc_publish_release_tree_events" do
  ...
  @doc "Builds the Tree event membership row for a Docs release."
  def changeset(%DocPublishReleaseTreeEvent{} = row, attrs) do
    row
