defmodule GroupherServer.CMS.Interactions.View do
  @moduledoc """
  Owns durable Article view recording and asynchronous projection.

      Article Reader -> record -> durable ViewEvent
      Oban worker    -> project/retry/retention -> Interaction State

  View recording is event-idempotent and intentionally does not acquire the
  Article aggregate MutationLock.
  """
  ...
  alias CMS.Interactions.{Config, ErrorCat, State}
  ...
  @spec record(struct(), User.t() | nil, Ecto.UUID.t() | nil) ::
          {:ok, Ecto.UUID.t()} | {:error, GroupherServer.ErrorCat.Error.t()}
  def record(article, viewer, event_id) do
    with {:ok, %{collection?: true}} <- Matcher.match_interaction(article) do
      case do_record(article, viewer, event_id) do
        {:ok, _event_id} = result -> result
        {:error, %GroupherServer.ErrorCat.Error{}} = result -> result
        _ -> {:error, ErrorCat.view_event_insert_failed()}
      end
    else
