defmodule GroupherServer.CMS.Interactions.Collect do
  @moduledoc """
  Executes idempotent Article collect commands.
  ...
  @doc "Collects an Article as an idempotent set-state command."
  @spec add(struct(), User.t()) :: T.domain_res(struct())
  def add(article, %User{} = actor), do: mutate(article, actor, :add)

  @doc "Removes an Article collect as an idempotent set-state command."
  ...
  defp mutate(input, actor, operation) do
    Repo.transaction(fn ->
      with {:ok, canonical} <- Gate.access_check(actor, :collect, input),
        ...
        {:ok, %{collection?: false}} -> Repo.rollback(ErrorCat.unsupported_artiment("Comment"))
  ...
  defp sync_state(canonical, actor, operation, :changed) do
    result =
      if operation == :add,
        do: State.add_collect(canonical, actor),
        else: State.remove_collect(canonical, actor)
