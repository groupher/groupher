defmodule GroupherServer.CMS.DocTree.Publish.PublicProjection do
  ...
  def preapply_tree_delete_events(%Community{} = community, events) do
    events
    |> Enum.filter(
      &(&1.event_type in [CMS.Const.tree_event(:node_delete), CMS.Const.tree_event(:pin_remove)])
    )
    |> apply_tree_events(community)
  end

  def apply_tree_events(events, %Community{} = community),
    do: apply_tree_events(community, events)

  def apply_tree_events(%Community{} = community, events) do
    events
    |> Enum.reduce_while(:ok, fn event, :ok ->
      case apply_tree_event(community, event) do
        :ok -> {:cont, :ok}
        {:ok, _node} -> {:cont, :ok}
        error -> {:halt, error}
      end
  ...
  def public_group_children(%Community{} = community, group_id) do
    DocTreeNode
  ...
  defp apply_tree_event(
    ...
    delete_public_node_by_node_id(community, node["id"], node["type"])
  end
  ...
  defp apply_tree_event(%Community{} = community, %DocTreeEvent{} = event) do
    apply_tree_event_fallback(community, event)
  end

  defp apply_tree_event_fallback(%Community{} = community, %DocTreeEvent{} = event) do
    payload = event.payload
  ...
  defp delete_public_node_by_node_id(%Community{} = community, node_id, node_type) do
    case public_node_by_node_id(community, node_id) do
