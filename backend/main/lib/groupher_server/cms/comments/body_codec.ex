defmodule GroupherServer.CMS.Comments.BodyCodec do
  @moduledoc """
  Temporary, Comment-only Plate codec.

  Article content is published by the Node artiment publisher. Comments have
  ...
  defp block_to_html(%{"type" => "toggle"} = node) do
    "<details#{html_id_attr(node)}><summary>#{inline_html(node)}</summary></details>"
  end
