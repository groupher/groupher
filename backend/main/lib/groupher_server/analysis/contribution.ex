defmodule GroupherServer.Analysis.Contribution do
  ...
  """
  def make_contribution(%User{id: id}) do
    ...
    with {:ok, user} <- ORM.find(User, id) do
      ...
      |> Multi.run(:update_community_field, fn _, _ ->
        {:ok, contributes} = list_contributions_digest(user)
  ...
  def make_contribution(%Community{id: id} = community) do
    today = Datetime.today() |> Date.to_iso8601()
  ...
  """
  def list_contributions_digest(%User{id: id}) do
    UserContribute
  ...
  def list_contributions_digest(%Community{id: id}) do
