defmodule GroupherServer.Repo.Migrations.BackfillLegacyUppercaseThreadValues do
  use Ecto.Migration
  ...
  defp normalize_article_meta_thread(table) do
    ...
    WHERE meta IS NOT NULL
      AND meta ? 'thread'
      AND meta->>'thread' IS NOT NULL
      AND meta->>'thread' != LOWER(meta->>'thread')
    """)
  end
  ...
  defp recreate_thread_match_constraint(table) do
    ...
    ADD CONSTRAINT #{table}_thread_matches_article_ref_check
    CHECK (
  ...
  defp drop_thread_match_constraint(table) do
    execute("""
    ALTER TABLE #{@cms_prefix}.#{table}
    DROP CONSTRAINT IF EXISTS #{table}_thread_matches_article_ref_check
    """)
  end
end
