defmodule GroupherServer.Test.Accounts.CollectFolder do
  ...
  describe "[collect folder curd]" do
    ...
    test "user can delete a empty collect folder", ~m(user)a do
      {:ok, folder} = CollectFolders.create(%{title: "test folder"}, user)
      {:ok, _} = CollectFolders.delete(folder.id)
    ...
    test "user can get public collect-folder list", ~m(user)a do
      {:ok, _folder} = CollectFolders.create(%{title: "test folder"}, user)
      {:ok, _folder} = CollectFolders.create(%{title: "test folder2"}, user)

      {:ok, result} = CollectFolders.paged(user.id, %{page: 1, size: 20})
    ...
    test "user can get public collect-folder list by thread", ~m(user post)a do
      ...
      {:ok, result} =
        CollectFolders.paged(user.id, %{thread: :post, page: 1, size: 20})
    ...
    test "user can not get private folder list of other user", ~m(user user2)a do
      {:ok, _folder} =
        CollectFolders.create(%{title: "test folder", private: true}, user2)

      {:ok, _folder} = CollectFolders.create(%{title: "test folder2"}, user2)

      {:ok, result} = CollectFolders.paged(user2.id, %{page: 1, size: 20}, user)
    ...
    test "collect creator can get both public and private folder list", ~m(user)a do
      {:ok, _folder} =
        CollectFolders.create(%{title: "test folder", private: true}, user)

      {:ok, _folder} = CollectFolders.create(%{title: "test folder2"}, user)
      {:ok, result} = CollectFolders.paged(user.id, %{page: 1, size: 20}, user)
    ...
    test "user can update a collect folder", ~m(user)a do
      ...
      {:ok, updated} = CollectFolders.update(folder.id, args)

      assert updated.desc == "new desc"
      assert updated.title == "new title"

      {:ok, updated} = CollectFolders.update(folder.id, %{private: true})
  ...
  describe "[add/remove from collect]" do
    ...
    test "one article collected in different collect-folder should only have one article-collect record",
         ~m(user post)a do
      {:ok, folder} = CollectFolders.create(%{title: "test folder"}, user)
      {:ok, folder2} = CollectFolders.create(%{title: "test folder2"}, user)
    ...
    test "can remove post to exist collect-folder", ~m(user post post2)a do
      {:ok, folder} = CollectFolders.create(%{title: "test folder"}, user)
      {:ok, _folder} = CollectFolders.add(post, folder.id, user)
      {:ok, _folder} = CollectFolders.add(post2, folder.id, user)

      {:ok, _} = CollectFolders.remove(post, folder.id, user)

      {:ok, result} =
        CollectFolders.paged_articles(folder.id, %{page: 1, size: 10}, user)
    ...
    test "add post to exist collect-folder should update meta", ~m(user post post2)a do
      ...
      {:ok, folder} = CollectFolders.remove(post, folder.id, user)
    ...
    test "remove post to exist collect-folder should update meta", ~m(user post post2)a do
      ...
      {:ok, folder} = CollectFolders.remove(post2, folder.id, user)
