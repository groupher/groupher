defmodule GroupherServer.Test.Payment do
  ...
  describe "[payment billing curd]" do
    test "create bill record with valid attrs", ~m(user valid_attrs)a do
      {:ok, record} = Payment.create_record(user, valid_attrs)
    ...
    test "create bill record with valid note", ~m(user valid_attrs)a do
      {:ok, record} = Payment.create_record(user, valid_attrs |> Map.merge(%{note: "i am girl"}))
    ...
    test "record state can be update", ~m(user valid_attrs)a do
      ...
      {:ok, updated} = Payment.update_record_state(record.id, :done)
    ...
    test "can get paged bill records of a user", ~m(user valid_attrs)a do
      ...
      {:ok, records} = Payment.paged_records(user, %{page: 1, size: 20})
  ...
  describe "[after billing]" do
    test "user updgrade to senior_member after senior bill handled", ~m(user valid_attrs)a do
      ...
      {:ok, record} = Payment.create_record(user, attrs)
      {:ok, _updated} = Payment.update_record_state(record.id, :done)
