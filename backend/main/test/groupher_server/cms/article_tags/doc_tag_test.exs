defmodule GroupherServer.Test.CMS.ArticleTag.DocTag do
  @moduledoc false
  ...
  describe "[doc tag CRUD]" do
    ...
    test "create article tag with extra & icon data", ~m(community article_tag_attrs user)a do
      ...
      {:ok, article_tag} = CMS.create_article_tag(community, :doc, tag_attrs, user)
  ...
  describe "[create/update doc with tags]" do
    test "can create doc with existed article tags",
      ...
      doc_with_tags =
        Map.merge(doc_attrs, %{article_tags: [article_tag.id, article_tag2.id]})

      {:ok, created} = CMS.create_article(community, :doc, doc_with_tags, user)
    ...
    test "can not create doc with other community's article tags",
      ...
      {:error, reason} = CMS.create_article(community, :doc, doc_with_tags, user)
