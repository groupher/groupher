defmodule GroupherServer.Test.CMS.AbuseReports.BlogReport do
  @moduledoc false

  use GroupherServer.TestMate

  setup do
    {_, blog, _, user} = mock_article(:blog)
    {:ok, user2} = db_insert(:user)

    {:ok, ~m(user user2 blog)a}
  end

  describe "[article blog report/unreport]" do
    test "list article reports should work", ~m(user user2 blog)a do
      {:ok, _} = CMS.AbuseReports.article(blog, "reason", "attr_info", user)
      {:ok, _} = CMS.AbuseReports.article(blog, "reason", "attr_info", user2)

      filter = %{content_type: :blog, content_id: blog.id, page: 1, size: 20}
      {:ok, all_reports} = CMS.AbuseReports.paged_reports(filter)

      report = all_reports.entries |> List.first()
      assert report.article.id == blog.id
      assert report.article.thread == :blog
    end

    test "report a blog should have a abuse report record", ~m(user blog)a do
      {:ok, _} = CMS.AbuseReports.article(blog, "reason", "attr_info", user)

      filter = %{content_type: :blog, content_id: blog.id, page: 1, size: 20}
      {:ok, all_reports} = CMS.AbuseReports.paged_reports(filter)

      report = List.first(all_reports.entries)
      report_cases = report.report_cases

      assert report.article.id == blog.id
      assert all_reports.total_count == 1
      assert report.report_cases_count == 1
      assert List.first(report_cases).user.login == user.login

      {:ok, blog} = CMS.Articles.InteractionResponse.one(blog, user, surface: :report)
      assert blog.meta.reported_count == 1
      assert blog.viewer_has_reported
    end

    test "can undo a report", ~m(user blog)a do
      {:ok, _} = CMS.AbuseReports.article(blog, "reason", "attr_info", user)
      {:ok, _} = CMS.AbuseReports.undo_article(blog, user)

      filter = %{content_type: :blog, content_id: blog.id, page: 1, size: 20}
      {:ok, all_reports} = CMS.AbuseReports.paged_reports(filter)
      assert all_reports.total_count == 0

      refute CMS.Interactions.viewer_state(blog, user).viewer_has_reported
    end

    test "can undo a existed report", ~m(user user2 blog)a do
      {:ok, _} = CMS.AbuseReports.article(blog, "reason", "attr_info", user)
      {:ok, _} = CMS.AbuseReports.article(blog, "reason", "attr_info", user2)
      {:ok, _} = CMS.AbuseReports.undo_article(blog, user)

      filter = %{content_type: :blog, content_id: blog.id, page: 1, size: 20}
      {:ok, all_reports} = CMS.AbuseReports.paged_reports(filter)
      assert all_reports.total_count == 1

      assert CMS.Interactions.viewer_state(blog, user2).viewer_has_reported
      refute CMS.Interactions.viewer_state(blog, user).viewer_has_reported
    end

    test "can undo a report with other user report it too", ~m(user user2 blog)a do
      {:ok, _} = CMS.AbuseReports.article(blog, "reason", "attr_info", user)
      {:ok, _} = CMS.AbuseReports.article(blog, "reason", "attr_info", user2)

      filter = %{content_type: :blog, content_id: blog.id, page: 1, size: 20}
      {:ok, all_reports} = CMS.AbuseReports.paged_reports(filter)
      assert all_reports.total_count == 1

      report = all_reports.entries |> List.first()
      assert report.report_cases |> length == 2
      assert Enum.any?(report.report_cases, &(&1.user.login == user.login))
      assert Enum.any?(report.report_cases, &(&1.user.login == user2.login))

      {:ok, _} = CMS.AbuseReports.undo_article(blog, user)

      filter = %{content_type: :blog, content_id: blog.id, page: 1, size: 20}
      {:ok, all_reports} = CMS.AbuseReports.paged_reports(filter)
      assert all_reports.total_count == 1

      report = all_reports.entries |> List.first()
      assert report.report_cases |> length == 1
      assert Enum.any?(report.report_cases, &(&1.user.login == user2.login))
    end

    test "different user report a comment should have same report with different report cases",
         ~m(user user2 blog)a do
      {:ok, _} = CMS.AbuseReports.article(blog, "reason", "attr_info", user)
      {:ok, _} = CMS.AbuseReports.article(blog, "reason2", "attr_info 2", user2)

      filter = %{content_type: :blog, content_id: blog.id, page: 1, size: 20}
      {:ok, all_reports} = CMS.AbuseReports.paged_reports(filter)

      report = List.first(all_reports.entries)
      report_cases = report.report_cases

      assert all_reports.total_count == 1
      assert length(report_cases) == 2
      assert report.report_cases_count == 2

      assert List.first(report_cases).user.login == user.login
      assert List.last(report_cases).user.login == user2.login
    end

    test "same user can not report a comment twice", ~m(blog user)a do
      {:ok, _} = CMS.AbuseReports.article(blog, "reason", "attr_info", user)

      assert {:error, _report} = CMS.AbuseReports.article(blog, "reason", "attr_info", user)
    end
  end
end
