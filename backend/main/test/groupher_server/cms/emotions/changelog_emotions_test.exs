defmodule GroupherServer.Test.CMS.Emotions.ChangelogEmotions do
  ...
  describe "[emotion in paged changelogs]" do
    test "login user should got viewer has emotioned status",
      ...
      {:ok, _} = CMS.emotion_to_article(random_changelog, :downvote, user)
      {:ok, _} = CMS.emotion_to_article(random_changelog, :beer, user)
      {:ok, _} = CMS.emotion_to_article(random_changelog, :popcorn, user)
  ...
  describe "[basic article emotion]" do
    ...
    test "can make emotion to changelog", ~m(community changelog_attrs user user2)a do
      ...
      {:ok, _} = CMS.emotion_to_article(changelog, :downvote, user)
      {:ok, _} = CMS.emotion_to_article(changelog, :downvote, user2)
    ...
    test "can undo emotion to changelog", ~m(community changelog_attrs user user2)a do
      ...
      {:ok, _} = CMS.undo_emotion_to_article(changelog, :downvote, user)
      {:ok, _} = CMS.undo_emotion_to_article(changelog, :downvote, user2)
    ...
    test "undo emotion without record is no-op", ~m(community changelog_attrs user)a do
      {:ok, changelog} = CMS.create_article(community, :changelog, changelog_attrs, user)
    ...
    test "same user same emotion to same changelog only have one user_emotion record",
      ...
      {:ok, _} = CMS.emotion_to_article(changelog, :downvote, user)
      {:ok, _} = CMS.emotion_to_article(changelog, :heart, user)
    ...
    test "different user can make same emotions on same changelog",
      ...
      {:ok, _} = CMS.emotion_to_article(changelog, :beer, user2)
      {:ok, _} = CMS.emotion_to_article(changelog, :beer, user3)
    ...
    test "same user can make different emotions on same changelog",
         ~m(community changelog_attrs user)a do
      {:ok, changelog} = CMS.create_article(community, :changelog, changelog_attrs, user)
      ...
      {:ok, _} = CMS.emotion_to_article(changelog, :beer, user)
      {:ok, _} = CMS.emotion_to_article(changelog, :heart, user)
      {:ok, _} = CMS.emotion_to_article(changelog, :orz, user)
