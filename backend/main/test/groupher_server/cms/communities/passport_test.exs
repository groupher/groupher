defmodule GroupherServer.Test.CMS.Communities.Passport do
  ...
  describe "[cms_passports]" do
    @valid_passport_rules %{
      "global" => %{},
      "communities" => %{"javascript" => %{"post.delete" => true, "post.edit" => true}}
    }
    @valid_passport_rules2 %{
      "global" => %{},
      "communities" => %{"javascript" => %{"post.pin" => true, "post.edit" => true}}
    }
    ...
    test "can insert valid nested passport structure", ~m(user)a do
      ...
      assert passport.rules |> get_in(["communities", "javascript", "post.delete"]) == true
      assert passport.rules |> get_in(["communities", "javascript", "post.edit"]) == true
    end
    ...
    test "false rules will not be delete from current passport", ~m(user)a do
      ...
      valid_passport2 = %{
        "global" => %{},
        "communities" => %{"javascript" => %{"post.edit" => false}}
      }

      {:ok, updated_passport} = Passport.stamp_passport(valid_passport2, user)

      assert updated_passport.user_id == user.id
      assert updated_passport.rules |> get_in(["communities", "javascript", "post.delete"]) == true
      assert updated_passport.rules |> get_in(["communities", "javascript", "post.edit"]) == nil
    end
    ...
    test "get a normal user's passport fails", ~m(user)a do
      assert {:ok, %{"global" => %{}, "communities" => %{}}} = Passport.get_passport(user)
    end
    ...
    test "can ease a rule in passport", ~m(user)a do
      ...
      {:ok, passport_after} = Passport.erase_passport(["communities", "javascript", "post.delete"], user)

      assert nil == passport_after.rules |> get_in(["communities", "javascript", "post.delete"])
    end
    ...
    test "can ease a rule in passport by community slug", ~m(user)a do
      ...
      {:ok, passport_after} = Passport.erase_passport(["communities", "javascript"], user)

      assert passport_after.rules == %{
               "global" => %{},
               "communities" => %{"elixir" => %{"post.delete" => true, "post.edit" => true}}
             }
    end
    ...
    test "erase a no-exist rule in passport is ok", ~m(user)a do
      ...
      {:ok, _} = Passport.erase_passport(["communities", "javascript", "non-exist"], user)
      {:ok, _} = Passport.erase_passport(["communities", "non-exist", "post.delete"], user)
      assert {:error, :invalid_passport_shape} = Passport.erase_passport(["non-exist", "non-exist"], user)
    end
  end
end
