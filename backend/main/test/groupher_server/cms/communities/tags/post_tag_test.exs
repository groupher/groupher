defmodule GroupherServer.Test.CMS.Communities.Tags.PostTagTest do
  ...
  describe "[post tag reindex]" do
    test "can reindex group of tags", ~m(community article_tag_attrs user)a do
      ...
      {:ok, article_tag1} = CMS.create_community_tag(community, :post, attrs, user)
      {:ok, article_tag2} = CMS.create_community_tag(community, :post, attrs, user)
      {:ok, article_tag3} = CMS.create_community_tag(community, :post, attrs, user)
      {:ok, article_tag4} = CMS.create_community_tag(community, :post, attrs, user)

      attrs = Map.merge(article_tag_attrs, %{group: "group2"})
      {:ok, article_tag5} = CMS.create_community_tag(community, :post, attrs, user)
      ...
      CMS.reindex_community_tags(community, :post, "group1", tags_with_index)
  ...
  describe "[post tag CRUD]" do
    ...
    test "create article tag with extra & icon data", ~m(community article_tag_attrs user)a do
      ...
      {:ok, article_tag} = CMS.create_community_tag(community, :post, tag_attrs, user)
    ...
    test "can update an article tag", ~m(community article_tag_attrs user)a do
      ...
      {:ok, article_tag} = CMS.update_community_tag(article_tag.id, new_attrs)
    ...
    test "tag can be deleted", ~m(community article_tag_attrs user)a do
      ...
      {:ok, _} = CMS.delete_community_tag(article_tag.id)
    ...
    test "assoc tag should be delete after tag deleted",
      ...
      {:ok, post} = CMS.set_community_tag(post, article_tag.id)
      {:ok, post} = CMS.set_community_tag(post, article_tag2.id)
      ...
      {:ok, _} = CMS.delete_community_tag(article_tag2.id)
  ...
  describe "[create/update post with tags]" do
    ...
    test "can not create post with other community's article tags",
      ...
      {:ok, article_tag} = CMS.create_community_tag(community, :post, article_tag_attrs, user)
      {:ok, article_tag2} = CMS.create_community_tag(community2, :post, article_tag_attrs2, user)
  ...
  describe "[post tag set /unset]" do
    test "can set a tag ", ~m(community post article_tag_attrs article_tag_attrs2 user)a do
      ...
      {:ok, post} = CMS.unset_community_tag(post, article_tag.id)
      assert post.article_tags |> length == 1
      assert not exist_in?(article_tag, post.article_tags)
      assert exist_in?(article_tag2, post.article_tags)

      {:ok, post} = CMS.unset_community_tag(post, article_tag2.id)
