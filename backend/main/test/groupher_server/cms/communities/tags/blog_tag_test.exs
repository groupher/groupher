defmodule GroupherServer.Test.CMS.Communities.Tags.BlogTagTest do
  ...
  describe "[blog tag CRUD]" do
    ...
    test "create article tag with extra & icon data", ~m(community article_tag_attrs user)a do
      ...
      {:ok, article_tag} = GroupherServer.CMS.Communities.create_tag(community, :blog, tag_attrs, user)
    ...
    test "assoc tag should be delete after tag deleted",
      ...
      {:ok, article_tag2} = GroupherServer.CMS.Communities.create_tag(community, :blog, article_tag_attrs2, user)

      {:ok, blog} = GroupherServer.CMS.Communities.set_tag(blog, article_tag.id)
      {:ok, blog} = GroupherServer.CMS.Communities.set_tag(blog, article_tag2.id)
  ...
  describe "[create/update blog with tags]" do
    ...
    test "can not create blog with other community's article tags",
      ...
      {:ok, article_tag2} = GroupherServer.CMS.Communities.create_tag(community2, :blog, article_tag_attrs2, user)
  ...
  describe "[blog tag set /unset]" do
    test "can set a tag ", ~m(community blog article_tag_attrs article_tag_attrs2 user)a do
      ...
      {:ok, blog} = GroupherServer.CMS.Communities.unset_tag(blog, article_tag.id)
      assert blog.article_tags |> length == 1
      assert not exist_in?(article_tag, blog.article_tags)
      assert exist_in?(article_tag2, blog.article_tags)

      {:ok, blog} = GroupherServer.CMS.Communities.unset_tag(blog, article_tag2.id)
