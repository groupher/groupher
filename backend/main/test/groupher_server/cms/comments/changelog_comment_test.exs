defmodule GroupherServer.Test.CMS.Comments.ChangelogComment do
  ...
  describe "[comments state]" do
    ...
    test "can get viewer joined state 2", ~m(community user user2 user3 changelog)a do
      {:ok, _} =
        CMS.create_comment2(community.slug, :changelog, changelog.inner_id, mock_comment(), user2)

      {:ok, _} =
        CMS.create_comment2(community.slug, :changelog, changelog.inner_id, mock_comment(), user3)
  ...
  describe "[article comment upvotes]" do
    ...
    # test "should auto subscribe community upvote a changelog comment", ~m(user changelog)a do
    #   {:error, _subscriber} =
    #     ORM.find_by(CommunitySubscriber, %{
    #       community_id: changelog.original_community_id,
    #       user_id: user.id
