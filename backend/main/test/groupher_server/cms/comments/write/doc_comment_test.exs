defmodule GroupherServer.Test.CMS.Comments.DocComment do
  ...
  setup do
    ...
    {:ok, ~m(community user user2 user3 doc cur_date)a}
  end
  ...
  describe "[basic article comment]" do
    ...
    test "create comment should update active timestamp of doc",
  ...
  describe "[article comment upvotes]" do
    ...
    test "user upvote doc comment will add id to upvoted_user_ids",
         ~m(community doc user)a do
      {:ok, comment} =
        CMS.Comments.create_comment(community, :doc, doc.inner_id, mock_comment(), user)
    ...
    test "user upvote a already-upvoted comment fails", ~m(community user doc)a do
      {:ok, comment} =
        CMS.Comments.create_comment(community, :doc, doc.inner_id, mock_comment(), user)
  ...
  describe "[article comment fold/unfold]" do
    test "user can fold a comment", ~m(community doc user)a do
      ...
      {:ok, comment} = CMS.Comments.fold_comment(comment.id, user)
      {:ok, comment} = ORM.find(Comment, comment.id)
    ...
    test "user can unfold a comment", ~m(community doc user)a do
      ...
      {:ok, _} = CMS.Comments.unfold_comment(comment.id, user)
      {:ok, comment} = ORM.find(Comment, comment.id)
      assert not comment.is_folded
  ...
  describe "paged article comments" do
    test "can load paged comments participants of a article", ~m(community user doc)a do
      total_count = 30
      page_size = 10
      thread = :doc
    ...
    test "paged article comments folded flag should be false", ~m(community user doc)a do
      total_count = 30
      page_number = 1
      page_size = 35
    ...
    test "paged article comments should not contains folded and reported comments",
      ...
      {:ok, _} = CMS.Comments.fold_comment(random_comment_1.id, user)
      {:ok, _} = CMS.Comments.fold_comment(random_comment_2.id, user)
      {:ok, _} = CMS.Comments.fold_comment(random_comment_3.id, user)
    ...
    test "can loaded paged folded comment", ~m(community user doc)a do
      total_count = 10
      page_number = 1
      page_size = 20
      ...
      {:ok, paged_comments} =
        CMS.Comments.paged_folded_comments(:doc, doc.id, %{page: page_number, size: page_size})
  ...
  describe "[lock/unlock doc comment]" do
    test "locked doc can not be comment", ~m(community user doc)a do
      ...
      {:ok, _} = CMS.Comments.lock_article_comments(doc)
      ...
      {:ok, _} = CMS.Comments.undo_lock_article_comments(doc)
