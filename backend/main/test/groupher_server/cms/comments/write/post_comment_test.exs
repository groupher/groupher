defmodule GroupherServer.Test.CMS.Comments.PostComment do
  ...
  describe "[article comment floor]" do
    ...
    test "create comment returns domain error when floor allocation fails", ~m(post user)a do
  ...
  describe "paged article comments" do
    ...
    test "can loaded paged folded comment", ~m(community user post)a do
      ...
      {:ok, paged_comments} =
        CMS.Comments.paged_folded_comments(:post, post.id, %{page: page_number, size: page_size})
