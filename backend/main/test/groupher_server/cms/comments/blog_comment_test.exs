defmodule GroupherServer.Test.CMS.Comments.BlogComment do
  ...
  describe "[comments state]" do
    ...
    # test "should subscribe community if not", ~m(user blog)a do
    #   {:error, _subscriber} =
    #     ORM.find_by(CommunitySubscriber, %{
    #       community_id: blog.original_community_id,
    #       user_id: user.id
    #     })
    ...
    #   assert subscriber.community_id === blog.original_community_id
    # end
    ...
    test "can get viewer joined state 2", ~m(community user user2 user3 blog)a do
      {:ok, _} =
        CMS.create_comment2(community.slug, :blog, blog.inner_id, mock_comment(), user2)

      {:ok, _} =
        CMS.create_comment2(community.slug, :blog, blog.inner_id, mock_comment(), user3)
  ...
  describe "[article comment upvotes]" do
    ...
    # test "should auto subscribe community upvote a blog comment", ~m(user blog)a do
    #   {:error, _subscriber} =
    #     ORM.find_by(CommunitySubscriber, %{
    #       community_id: blog.original_community_id,
    #       user_id: user.id
