defmodule GroupherServer.Test.CMS.PostPendingFlag do
  ...
  describe "[pending posts flags]" do
    test "pending post can not be read", ~m(post_m)a do
      {:ok, _} = CMS.read_article(post_m.community_slug, :post, post_m.inner_id)
      ...
      {:error, reason} = CMS.read_article(post_m.community_slug, :post, post_m.inner_id)
    ...
    test "author can read it's own pending post", ~m(community user)a do
      ...
      {:ok, _} = CMS.read_article(post.community_slug, :post, post.inner_id)
      ...
      {:ok, post_read} =
        CMS.read_article(post.community_slug, :post, post.inner_id, user)
      ...
      {:error, reason} =
        CMS.read_article(post.community_slug, :post, post.inner_id, user2)
