defmodule GroupherServer.Test.CMS.BlogPendingFlag do
  ...
  describe "[pending blogs flags]" do
    test "pending blog can not be read", ~m(blog_m)a do
      {:ok, _} = CMS.Articles.read(blog_m.community_slug, :blog, blog_m.inner_id)
      ...
      {:error, reason} = CMS.Articles.read(blog_m.community_slug, :blog, blog_m.inner_id)
    ...
    test "author can read it's own pending blog", ~m(community user)a do
      ...
      {:ok, _} = CMS.Articles.read(blog.community_slug, :blog, blog.inner_id)
      ...
      {:ok, blog_read} =
        CMS.Articles.read(blog.community_slug, :blog, blog.inner_id, user)
      ...
      {:error, reason} =
        CMS.Articles.read(blog.community_slug, :blog, blog.inner_id, user2)
