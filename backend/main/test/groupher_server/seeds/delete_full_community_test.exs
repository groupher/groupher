defmodule GroupherServer.Test.Seeds.DeleteFullCommunityTest do
  ...
  describe "[delete full community seeds]" do
    test "delete_full_community removes related records" do
      ...
      changelog_ids = Repo.all(from(p in Changelog, where: p.community_id == ^community.id, select: p.id))
      ...
      assert Repo.aggregate(from(c in CommunityDashboard, where: c.community_id == ^community.id), :count) > 0
      assert Repo.aggregate(from(c in CommunityTag, where: c.community_id == ^community.id), :count) > 0
      assert Repo.aggregate(from(c in CommentUpvote, where: c.comment_id in ^comment_ids), :count) > 0
      assert Repo.aggregate(from(c in CommentUserEmotion, where: c.comment_id in ^comment_ids), :count) > 0
      ...
      assert Repo.aggregate(from(c in CommentReply, where: c.comment_id in ^comment_ids), :count) == 0
      assert Repo.aggregate(from(c in CommentUpvote, where: c.comment_id in ^comment_ids), :count) == 0
      assert Repo.aggregate(from(c in CommentUserEmotion, where: c.comment_id in ^comment_ids), :count) == 0
      ...
      assert Repo.aggregate(
               from(a in ArticleUpvote,
                 where:
                   a.post_id in ^post_ids or a.changelog_id in ^changelog_ids or a.doc_id in ^doc_ids
               ),
               :count
             ) == 0
