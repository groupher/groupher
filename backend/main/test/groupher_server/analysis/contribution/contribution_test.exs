defmodule GroupherServer.Test.Analysis.Contribution do
  @moduledoc false
  ...
  describe "[analysis contribution user] " do
    ...
    test "list_contributes return proper format ", ~m(user)a do
      Analysis.make_contribution(%User{id: user.id})
      {:ok, contributes} = Analysis.list_contributions_digest(%User{id: user.id})
      # contributes[0]
  ...
  describe "[analysis contribution community] " do
    test "should inserted a community contribute when create community", ~m(community)a do
      ...
      Analysis.make_contribution(%Community{id: community.id})
    ...
    test "should return recent #{@community_contribute_days} days community contributes by default",
      ...
      {:ok, contributes} = Analysis.list_contributions_digest(%Community{id: community.id})
