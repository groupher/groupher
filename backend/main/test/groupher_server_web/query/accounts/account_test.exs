defmodule GroupherServer.Test.Query.Account.Basic do
  ...
  describe "[account basic]" do
    ...
    test "login newbie user can get own empty cms_passport", ~m(user)a do
      ...
      assert results["cmsPassport"] == %{"global" => %{}, "communities" => %{}}
      assert Jason.decode!(results["cmsPassportString"]) == %{"global" => %{}, "communities" => %{}}
    end
    ...
    test "login user can get empty if cms_passport not exist", ~m(user)a do
      ...
      assert %{"global" => %{}, "communities" => %{}} == results["cmsPassport"]
      assert Jason.decode!(results["cmsPassportString"]) == %{"global" => %{}, "communities" => %{}}
    end
