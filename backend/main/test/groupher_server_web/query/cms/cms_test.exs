defmodule GroupherServer.Test.Query.CMS.Basic do
  ...
  describe "[cms query community]" do
    ...
    test "user can get community info without args fails", ~m(guest_conn user)a do
      ...
      {:ok, community} = CMS.Communities.create(community_attrs, user)
      {:ok, _} = CMS.Dsb.update(community, :seo, %{og_title: "groupher"})
      ...
      {:ok, _} =
        CMS.Dsb.update(community, :layout, %{kanban_bg_colors: [:green, :red]})

      {:ok, _} =
        CMS.Dsb.update(community, :base_info, %{favicon: "new favicon"})
