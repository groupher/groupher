defmodule GroupherServer.Test.Query.PagedArticles.PagedBlogs do
  ...
  describe "[query paged_blogs filter pagination]" do
    ...
    test "comments_count order should work",
      ...
      {:ok, _} = CMS.create_comment2(community, :blog, blog_id, mock_comment(), user)
      {:ok, _} = CMS.create_comment2(community, :blog, blog_id, mock_comment(), user2)
      {:ok, _} = CMS.create_comment2(community, :blog, blog_id, mock_comment(), user3)
  ...
  describe "[query paged_blogs filter has_xxx]" do
    ...
    test "has_xxx state should work", ~m(user)a do
      ...
      {:ok, blog} = CMS.create_article(community, :blog, mock_attrs(:blog), user)
      {:ok, _blog2} = CMS.create_article(community, :blog, mock_attrs(:blog), user)
  ...
  describe "[query paged_blogs filter when]" do
    ...
    test "THIS_MONTH option should work", ~m(guest_conn blog_last_month)a do
      ...
      assert results["entries"] |> Enum.any?(&(&1["id"] != blog_last_month.id))
    end
  end
