defmodule GroupherServer.Test.Query.PagedArticles.PagedDocs do
  ...
  describe "[query paged_docs filter pagination]" do
    ...
    test "views order should work", ~m(guest_conn community user user2 user3)a do
      ...
      {:ok, _} =
        CMS.read_article(doc.community_slug, :doc, doc.inner_id, user)

      {:ok, _} =
        CMS.read_article(doc.community_slug, :doc, doc.inner_id, user2)

      {:ok, _} =
        CMS.read_article(doc.community_slug, :doc, doc.inner_id, user3)
