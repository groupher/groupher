defmodule GroupherServer.Test.Query.Comments.DocComment do
  ...
  test "can get one comment by ref", ~m(guest_conn community doc user)a do
    thread = :doc
  ...
  test "can get one comment by ref with viewer states", ~m(user_conn community doc user)a do
    thread = :doc
  ...
  describe "paged replies" do
    ...
    test "guest user can get paged replies", ~m(guest_conn community doc user user2)a do
      ...
      variables = %{
        comment: comment_path(community, doc, :doc, parent_comment),
        filter: %{page: 1, size: page_size}
      }
