defmodule GroupherServer.Test.Query.Comments.ChangelogComment do
  ...
  test "can get one comment by ref", ~m(guest_conn community changelog user)a do
    thread = :changelog
  ...
  test "can get one comment by ref with viewer states", ~m(user_conn community changelog user)a do
    thread = :changelog
  ...
  describe "[basic article changelog comment]" do
    ...
    test "timeline-mode paged comments", ~m(guest_conn community changelog user user2)a do
      total_count = 3
      page_size = 20
      thread = :changelog
    ...
    test "guest user can get paged comment for changelog",
    ...
    test "user make emotion can get paged comment with emotions has_motioned field",
         ~m(user_conn community changelog user user2)a do
      total_count = 10
      page_size = 12
      thread = :changelog
