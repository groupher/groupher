defmodule GroupherServer.Test.Mutation.ArticleCommunity.Doc do
  ...
  describe "[mirror/unmirror/move doc to/from community]" do
    ...
    test "auth user can mirror doc home", ~m(user community doc)a do
      ...
      {:ok, doc} = ORM.find(Doc, doc.id, preload: [:communities, :community_tags])
    ...
    test "auth user can move doc to blackhole", ~m(community blackhole doc)a do
      ...
      {:ok, doc} = ORM.find(Doc, doc.id, preload: [:community, :communities, :community_tags])

      assert doc.community.id == blackhole.id
    end

    test "auth user can move doc to other community", ~m(community community2 doc)a do
      passport_rules = %{"doc.community.mirror" => true}
      rule_conn = simu_conn(:user, cms: passport_rules)
      ...
      {:ok, found} = ORM.find(Doc, doc.id, preload: [:community, :communities, :community_tags])
