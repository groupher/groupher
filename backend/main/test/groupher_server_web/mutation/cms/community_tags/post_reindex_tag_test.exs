defmodule GroupherServer.Test.Mutation.CommunityTags.PostReindexTag do
  ...
  describe "[mutation post tag]" do
    ...
    test "auth user can reindex tags in given group", ~m(community community_tag_attrs user)a do
      ...
      {:ok, community_tag1} = GroupherServer.CMS.Communities.create_tag(community, :post, attrs, user)
      {:ok, community_tag2} = GroupherServer.CMS.Communities.create_tag(community, :post, attrs, user)
      {:ok, community_tag3} = GroupherServer.CMS.Communities.create_tag(community, :post, attrs, user)
      {:ok, community_tag4} = GroupherServer.CMS.Communities.create_tag(community, :post, attrs, user)
