defmodule GroupherServer.Test.Mutation.CommunityTags.PostReindexTag do
  ...
  describe "[mutation post tag]" do
    ...
    test "auth user can reindex tags in given group", ~m(community community_tag_attrs user)a do
      ...
      {:ok, community_tag1} = CMS.create_community_tag(community, :post, attrs, user)
      {:ok, community_tag2} = CMS.create_community_tag(community, :post, attrs, user)
      {:ok, community_tag3} = CMS.create_community_tag(community, :post, attrs, user)
      {:ok, community_tag4} = CMS.create_community_tag(community, :post, attrs, user)
