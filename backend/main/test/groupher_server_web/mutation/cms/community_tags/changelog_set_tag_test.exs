defmodule GroupherServer.Test.Mutation.CommunityTags.ChangelogSetTag do
  ...
  describe "[mutation changelog tag]" do
    test "auth user can set a valid tag to changelog",
      ...
      {:ok, community_tag} = CMS.create_community_tag(community, :changelog, community_tag_attrs, user)
    ...
    test "can unset tag to a changelog",
      ...
      {:ok, community_tag2} =
        CMS.create_community_tag(community, :changelog, community_tag_attrs2, user)

      {:ok, _} = CMS.set_community_tag(changelog, community_tag.id)
      {:ok, _} = CMS.set_community_tag(changelog, community_tag2.id)
