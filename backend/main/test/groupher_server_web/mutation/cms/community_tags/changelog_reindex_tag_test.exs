defmodule GroupherServer.Test.Mutation.CommunityTags.ChangelogReindexTag do
  ...
  describe "[mutation changelog tag]" do
    ...
    test "auth user can reindex tags in given group", ~m(community community_tag_attrs user)a do
      ...
      {:ok, community_tag1} = CMS.create_community_tag(community, :changelog, attrs, user)
      {:ok, community_tag2} = CMS.create_community_tag(community, :changelog, attrs, user)
      {:ok, community_tag3} = CMS.create_community_tag(community, :changelog, attrs, user)
      {:ok, community_tag4} = CMS.create_community_tag(community, :changelog, attrs, user)
