defmodule GroupherServer.Test.Mutation.ArticleTags.DocReindexTag do
  @moduledoc false
  ...
  setup do
    ...
    {:ok, ~m(user_conn guest_conn owner_conn community doc article_tag_attrs user)a}
  end
  ...
  describe "[mutation doc tag]" do
    ...
    test "auth user can reindex tags in given group", ~m(community article_tag_attrs user)a do
      ...
      {:ok, article_tag1} = CMS.create_article_tag(community, :doc, attrs, user)
      {:ok, article_tag2} = CMS.create_article_tag(community, :doc, attrs, user)
      {:ok, article_tag3} = CMS.create_article_tag(community, :doc, attrs, user)
      {:ok, article_tag4} = CMS.create_article_tag(community, :doc, attrs, user)
