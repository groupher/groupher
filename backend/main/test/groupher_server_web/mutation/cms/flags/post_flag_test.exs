defmodule GroupherServer.Test.Mutation.Flags.PostFlag do
  ...
  describe "[mutation post flag curd]" do
    test "auth user can markDelete post", ~m(community post)a do
      ...
      updated =
        rule_conn |> gq_mutation(S.Article.m(:mark_delete_article, :post), variables)
    ...
    test "unauth user markDelete post fails",
      ...
      schema = S.Article.m(:mark_delete_article, :post)
    ...
    test "auth user can undo markDelete post", ~m(community post)a do
      ...
      updated =
        rule_conn |> gq_mutation(S.Article.m(:undo_mark_delete_article, :post), variables)
    ...
    test "unauth user undo markDelete post fails",
      ...
      schema = S.Article.m(:undo_mark_delete_article, :post)
    ...
    test "auth user can batch mark delete posts",
      ...
      variables = %{
        ...
        innerIds: [post.inner_id, post2.inner_id]
      }
      ...
      updated =
        rule_conn |> gq_mutation(S.Article.m(:batch_mark_delete_article, :post), variables)
    ...
    test "auth user can batch undo mark delete posts",
      ...
      updated =
        rule_conn |> gq_mutation(S.Article.m(:batch_undo_mark_delete_article, :post), variables)
    ...
    test "auth user can pin post", ~m(community post)a do
      ...
      updated = rule_conn |> gq_mutation(S.Article.m(:pin_article, :post), variables)
    ...
    test "unauth user pin post fails", ~m(user_conn guest_conn community post)a do
      ...
      assert user_conn
             |> mutation_error?(S.Article.m(:pin_article, :post), variables, ecode(:passport))

      assert guest_conn
             |> mutation_error?(
               S.Article.m(:pin_article, :post),
               variables,
               ecode(:account_login)
             )
    ...
    test "auth user can undo pin post", ~m(community post)a do
      ...
      updated = rule_conn |> gq_mutation(S.Article.m(:undo_pin_article, :post), variables)
    ...
    test "unauth user undo pin post fails", ~m(user_conn guest_conn community post)a do
      ...
      assert user_conn
             |> mutation_error?(
               S.Article.m(:undo_pin_article, :post),
               variables,
               ecode(:passport)
             )
