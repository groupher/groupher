defmodule GroupherServer.CMS.Model.DocCoverItem do
  @moduledoc """
  One page shown inside a docs cover card.
  ...
  @required_fields ~w(community_id cover_card_id node_id index)a
  @optional_fields ~w(hidden appearance)a

  @type t :: %DocCoverItem{}
  schema "doc_cover_items" do
    belongs_to(:community, Community)
    belongs_to(:cover_card, DocCoverCard)
  ...
  def changeset(%DocCoverItem{} = item, attrs) do
    ...
    |> foreign_key_constraint(:cover_card_id)
    |> foreign_key_constraint(:node_id)
    |> unique_constraint(:node_id, name: :doc_cover_items_cover_card_node_index)
  end
