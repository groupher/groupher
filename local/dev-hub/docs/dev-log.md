...
- 展示 Git 状态和 patch；
- 查看服务配置及依赖关系；
- 安全重启可识别的旧进程，并明确区分受管进程和无法确认归属的外部进程；
...
`ServiceManager` 不把“端口健康”直接等同于“服务已受管”。启动或初始化时，它会先检查端口监听 PID，再结合进程组、工作目录、运行时命令和服务定义判断是否可以安全接管：

1. 如果可以确认进程属于当前服务，先终止旧进程组；
2. 等待服务声明的端口释放；
3. 按服务的 `startPolicy` 启动必需依赖和目标服务；
4. 用 Dev Hub 自己创建的新子进程接收 stdout / stderr，并通过 SSE 推送终端日志。

这一步不能只把旧进程状态改成 `running`。旧进程没有挂在当前 `ServiceManager` 的 `ChildProcess` 上，Dev Hub 就没有可靠的日志管道，也无法正确执行停止和退出清理。因此，重新打开 Dev Hub 后，对于可确认归属的旧应用，预期行为是“关闭旧进程并重新打开”，而不是长期保持无日志的 `External` 或伪受管状态。

如果端口被占用但无法确认归属，服务保持 `external`。例如另一个项目的 Vite/Node 进程、从不同目录手动启动的服务、代理或容器监听了相同端口，或者进程命令和服务定义不匹配。此时不自动发送信号，避免误杀不属于 Groupher 的程序；开发者需要从原终端或所属工具关闭它，再回到 Dev Hub 启动服务。

`external` 不是不可诊断状态。只要监听进程信息可读，服务卡片上的 `External` 状态就可以打开 External Drawer，查看端口、PID、进程组、工作目录和命令。Drawer 始终保留手动关闭入口，但只有匹配到当前服务定义的安全进程组时按钮才可用；无法确认归属时按钮禁用，并明确提示从原终端或所属工具关闭。列表卡片和 Flow 视图复用同一个入口和停止接口。

服务启动发现和手动 `Start` / `Restart` 共用这套边界。依赖关系仍由 `src/server/services.ts` 的 `startPolicy` 声明，不能因为目标应用端口已经响应就跳过必需的 Gateway、Auth、Phoenix 或其他依赖。
