...
const listPatternSvgFiles = () =>
  existsSync(patternDir)
    ? readdirSync(patternDir)
        .filter((file) => path.extname(file).toLowerCase() === svgExtension)
        .sort((a, b) => a.localeCompare(b))
    : []
...
const ensurePattern = async (file) => {
  const source = path.join(patternDir, file)
  const targetFile = `${path.basename(file, svgExtension)}.png`
  const target = path.join(patternDir, targetFile)

  const pattern = await sharp(source, { failOn: 'none' })
    .resize({
      width: patternWidth,
      withoutEnlargement: true,
    })
  ...
  writeFileSync(target, pattern)

  return targetFile
}

const removeStalePatternPngs = (generatedPatternFiles) => {
  const keepFiles = new Set(generatedPatternFiles)

  for (const file of listPatternPngFiles()) {
    if (keepFiles.has(file)) continue

    unlinkSync(path.join(patternDir, file))
  }
}
...
const generatedPatternFiles = await Promise.all(listPatternSvgFiles().map(ensurePattern))
removeStalePatternPngs(generatedPatternFiles)
