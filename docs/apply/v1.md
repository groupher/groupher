...
5. 创建 Root/Owner 关系和根 Passport。
6. 创建最小 CommunityLifecycle，状态为 `setting_up`。
7. 将 Slug Claim 转为 Community ownership。
8. 将 Application 更新为 `setting_up`、关联 `community_id` 并投影 Community Logo。
9. 插入 Application Event。
10. 在同一数据库事务中插入唯一 Community Setup Oban Job。
...
- Community Dashboard 默认配置。
- 默认板块和基础 taxonomy。
- Docs Tree 初始化。
- 必要的搜索、缓存和公开 projection。
- Web Analysis 等允许 best-effort/retry 的外部能力。
- 创建完成通知。
