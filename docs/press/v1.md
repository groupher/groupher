...
当前仓库已经具备部分可复用能力，但还没有 Press 专用的 internal projection/API：

- Docs Markdown 当前通过 `getDoc -> public GraphQL -> document.markdown` 读取；该路径可能进入
  public Article read path，不适合作为 Press 的长期机器输出合同。
- 当前 public Article、ArticleDocument、published Docs tree 和 active Doc release 已经存在，但尚未
  封装为 Press DTO，也不能让 Press 直接查询业务表。
- `ArticleSnapshot` 是 Phoenix 的历史记录，不进入 Press public contract；Press 不接受
  `snapshot_ref` 或历史 release 参数，只读取当前最新的 public 内容。
- `CMS.Press.article/community_rss_feed/thread_rss_feed/site_manifest` 是 v1 需要新增的
  内部读取合同，负责一次性完成 public stage、可见性、Thread 和数量限制等业务过滤。
...
- community/custom-domain、locale、canonical origin。
...
Absinthe 的默认 camel-case adapter 会把字段缩写输出为 `Rss`，因此字段使用可执行的
`pressCommunityRssFeed / pressThreadRssFeed`；DTO 类型仍明确命名为 `*RSSFeed`。Press 客户端
可用 GraphQL alias 将响应键保持为 `pressCommunityRSSFeed / pressThreadRSSFeed`。
...
Press 可以缓存 Phoenix 接口返回的 current public projection 和最终输出，但不能直接查询
Phoenix 的 `cms` 业务 schema。缓存是读取接口之上的执行层，不是绕过接口的数据副本。
v1 中 Press 与 Phoenix 共用同一个 PostgreSQL database；Press 使用 Drizzle 直接管理和读写独立的
`analysis` schema（包括 output cache 与统计表），不通过 Ecto migration，也不影响 `cms` 业务事务。
...
v1 的 custom-domain community 映射先由 Gateway 的 `CUSTOM_DOMAIN_COMMUNITIES` JSON 环境变量
提供（`host -> community slug`），用于将 custom-domain 根路径的 Press 请求改写为内部 community
scope。当前 Phoenix 尚无可复用的 custom-domain registry，因此 canonical origin 仍由 Phoenix 的
`site_host` projection 返回；上线真实 custom domain 前，需要把域名注册表落到 Phoenix 的领域模型，
再让 Gateway 和 `CMS.Press` 读取同一事实源。Press 不直接信任公网 `Host` header 推导 canonical URL。
...
| Blog Markdown      | `/{community}/blog/{id}.md`       |
...
- canonical origin 来自 Phoenix 已验证的 domain 配置，不能直接信任 Host header。
...
- Docs Feed 使用当前 public Doc 的稳定 article ref 作为 GUID；历史 Doc revision 和旧 release
  不会成为可访问的 Feed item。
...
v1 的 L2 已选择 PostgreSQL `analysis.press_output_cache`，由 Drizzle 管理；业务代码仍只依赖
Press cache interface，不直接耦合数据库 API。以后替换为 KV、Redis 或对象存储时不改变 renderer。
...
Press 已提供 community-scoped internal invalidation endpoint，供以下变化主动失效 pointer；v1
先以 60 秒 pointer TTL 作为所有发布路径的正确性保证，因此即使某个 Phoenix hook 尚未接线也会
在有界时间内刷新：

- content publish/republish。
- Trash、unpublish、archive、permanent delete。
- Docs release publish。
- Press config update。
- custom domain/canonical origin update。

无 durable Outbox 时不能承诺即时、永久一致的 CDN purge，因此 v1 不使用无限长的
current-path cache。Phoenix lifecycle hooks 的主动调用可以逐步补齐，但不能替代 TTL；v2 再将
invalidation signal 升级为事务 Outbox + 可重试任务。
...
为了支持“某篇文章的趋势 + bot 明细 + 时间段追踪”，v1 允许并建议保留一层轻量事件明细：
...
建议在 v1b（P1）里补齐落库能力：
`MetricEvent` 异步写入 + `MetricHourly` rollup + `bot_family` 粒度。

### 存储落地建议（TSDB 取舍）

统计属于时序事件，但 v1 可以先不马上上专用 TSDB：

- Press 与 Phoenix 可以共用 PostgreSQL database，但统计持久化统一归入 `analysis` analytics namespace，
...
- `MetricEvent`（原始明细）建议短留存（如 7 天）；
- `MetricHourly`（按小时聚合）长期保留（如 180 天），承担趋势查询主力。
...
- unknown/hidden/deleted content：`404`，negative cache 使用短 TTL。
- Phoenix timeout/unavailable 且没有安全 cache：`502/503`，不能伪装成 `404`。
- v1 未建立可靠 purge/outbox 前不发送 `stale-if-error`，避免撤回或权限收紧后由中间缓存继续
  返回旧公开内容；后续只有在 lifecycle freshness 可证明时才能开启。
...
### v1a：边界和通用 Markdown（Must）

- 建立 `CMS.Press` config facade 和无副作用 current public projection。
- 建立 `backend/Press` Hono app、health 和 cache interface；v1 不实施 service credential 校验。
...
- 建立 `MetricEvent` 异步落库 + `MetricHourly` rollup（P1），用于内容级趋势与 bot 分类分析。

### v1c：Agent 与 SEO 输出（Must）
...
- 输出总大小限制为 5 MiB，site manifest/sitemap v1 最多 500 项；超过该规模后通过后续
  sitemap index/shard 版本扩展，不能静默截断为“完整站点”。
...
- publish、delete、config、domain 变化可以使 current output 在有界时间内失效。
