...
>
> 当前状态：v1 已部署到真实线上环境，正式域名与 Gateway 路由已完成冒烟验收
...
- 线上服务 origin：`https://press.groupher.com`。
- 面向用户的 URL 仍属于社区域名，由 Gateway 转发，不能暴露内部服务域名。

这里的名称属于不同层级：`@groupher/press` 是 monorepo workspace package，
`groupher-press` 是不能使用 npm scope 形式命名的 Fly 基础设施资源，`press.groupher.com`
是 Gateway 和 Phoenix 调用的稳定服务域名。三者不能混用，也不另建面向用户的 Press URL
体系。
...
- `DocSnapshot` 是 Phoenix 的 Doc 历史记录，不进入 Press public contract；Press 不接受
...
旧的 Main Markdown Route Handler 只在切换期间作为兼容入口。真实线上
`Gateway -> Press -> Phoenix` 链路通过验收后删除，不能让 Main 和 Press 长期各维护一套
renderer。删除范围只包括旧的 HTTP route/renderer；Phoenix 持久化 Markdown、
ArticleDocument 读取和 `CMS.Press.article` projection 仍是 Press 的 canonical 数据来源，必须保留。
...
- `DocSnapshot` 等历史记录继续由 Phoenix 管理，但不会通过 Press 接口暴露。
...
Press 的 `POST /internal/invalidate` 必须携带 Auth 签发给
`aud=press:internal-api`、`scope=press:cache:invalidate` 的 Phoenix service token。Phoenix 在 Article publish/republish 和 Press config update
...
- Press-to-Phoenix 读取使用 `sub=service:press`、`aud=phoenix:press-api` 的
  operation-scoped service token。
- Phoenix-to-Press 的 `/internal/invalidate` 使用
  `sub=service:phoenix`、`aud=press:internal-api`、
  `scope=press:cache:invalidate` 的 service token。
- v1 Phoenix operations 只能返回有界的当前 public 内容，不提供 Draft、private、历史 revision、
  任意 release 或 mutation 能力。
- Press 不传播浏览器 Cookie 或 Authorization；v2 统一采用 `docs/auth/v2.md` 定义的
...
v1 将 `@groupher/press` 作为 Docker 化的常驻 Node 服务部署到 Fly.io，Fly app id 为
`groupher-press`，与 Phoenix 一样优先运行在 `sin` region。

选择 Fly.io 不是因为 Cloudflare 或 Vercel 不能运行容器，而是因为 Press 当前直接依赖：

- 常驻的 Node/Hono HTTP server。
- PostgreSQL TCP 连接和 Drizzle migration。
- 进程内 L1 cache，以及 PostgreSQL 中的持久化 output cache。
- 响应后的异步 metrics 写入和进程内 retention timer。

Fly 可以在不改写上述运行模型的情况下直接运行 Docker image，并与现有 Phoenix Fly app
保持相同的部署和区域模型。Cloudflare Containers 或 Vercel Docker 后续仍可评估，但 v1
不为了更换平台先重写数据库连接、后台任务或进程生命周期。

```text
Cloudflare Pages Worker
  groupher.com/:community/*.md|feed.*|llms.txt|sitemap.xml
                      |
                      | PRESS_SITE=https://press.groupher.com
                      v
             Fly app: groupher-press
                      |
          +-----------+-----------+
          |                       |
          v                       v
 Phoenix GraphQL             PostgreSQL analysis
 CMS.Press projection        cache + metrics
```

`press.groupher.com` 是稳定的服务 origin，应绑定到 `groupher-press` Fly app。Fly 默认的
`groupher-press.fly.dev` 只用于平台诊断和 custom-domain 故障排查，不能写入正式的
`PRESS_SITE` 或 `PRESS_INTERNAL_URL`。面向用户和 Feed reader 的 canonical URL 仍是
`groupher.com/:community/...` 或已验证的 community custom domain。
...
`DATABASE_URL` 对应的数据库角色只需管理 `analysis.press_*` 对象，不能把 Press 的内容读取
改成直接访问 `cms`。生产环境缺少 `DATABASE_URL` 或 `PHOENIX_GRAPHQL_ENDPOINT` 时不能仅凭
`/health` 判断部署成功；必须继续执行真实内容和 metrics 冒烟验证。

### 上线顺序

1. 部署 Phoenix migration/projection，确认生产 GraphQL 已存在 Press 查询。
2. 执行 Press Drizzle migration，建立 `analysis.press_*` 表。
3. 部署 `groupher-press`，绑定并验证 `press.groupher.com`。
4. 通过 `press.groupher.com` 直接验证 Press 到生产 Phoenix/PostgreSQL 的链路。
5. `edge-router` 在 Community 通用路径前识别 Press 公共输出，并将 `PRESS_SITE` 指向
   `https://press.groupher.com`；不存在 Main fallback。
6. 部署真实 `groupher.com` 路由并完成下列验收。
7. 验收通过后删除旧 Main Doc `.md` HTTP route/renderer。

首轮直接连接真实线上环境，不以完整 staging 或全自动 E2E 作为上线前置条件，但必须验证：

- 一篇真实 published Doc 的 `.md` 正文和 Content-Type。
- RSS、Atom、JSON Feed、`llms.txt` 和 community sitemap。
- unknown/hidden 内容为 `404`，Phoenix unavailable 为 `502/503`。
- ETag 条件请求返回 `304`，重复读取能复用 cache。
- `.md`/Feed 请求不会增加 Article human views。
- `analysis.press_metric_events` 和 `analysis.press_metric_hourly` 有真实记录。
- Press config update/publish 后的 invalidation 能更新输出。

### Dev Hub

Dev Hub 的 Press service card 应映射到 Fly deployment target，显示 Fly logo，并链接到
`https://fly.io/apps/groupher-press`；Fly infrastructure links 同时增加 Press。Landing 已迁到
Cloudflare Pages，因此其现有 Vercel platform 标识也应在同一轮更新为 Cloudflare，避免本地
拓扑显示与生产部署事实不一致。
