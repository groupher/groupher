...
- 面向浏览器的 GraphQL 调用使用同源相对路径 `/api/graphql`，不再配置 public GraphQL endpoint。
...
浏览器 GraphQL 地址固定为 `/api/graphql`，由 Next route handler 或 Gateway 转发到
Phoenix；浏览器代码不读取 `NEXT_PUBLIC_GRAPHQL_ENDPOINT`。
