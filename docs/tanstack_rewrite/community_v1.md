...
> 历史迁移记录：下文的 Main/Community 并行约束描述首期重写阶段。`frontend/main` 现已
> 删除，Community 是唯一公开社区应用。
...
| Community GraphQL 的 `inc_views` 默认值是 `true`                                 | `backend/api/lib/groupher_server_web/schema/cms/cms_queries.ex`                        | 冻结目标计数语义，不能把“未显式传参”解释成 false                          |
| Community Reader 默认执行 views increment                                        | `backend/api/lib/groupher_server/cms/communities/reader.ex`                            | 明确 cache miss 与 community views 的现状关系                             |
| Article detail read 调用 `Interactions.record_view`                              | `backend/api/lib/groupher_server/cms/articles/reader.ex`                               | 明确 detail cache hit/miss 与 view event 的现状关系                       |
