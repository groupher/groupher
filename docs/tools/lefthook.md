...
当前主要问题不是 Husky 本身，而是 pre-commit 在 staged 检查后还执行全仓 yarn format:check，容易让无关脏文件阻塞小提交。迁移的真正价值是：
...
type-check-core:
  ...
  run: yarn tsc-files --noEmit --project frontend/core/tsconfig.app.json frontend/core/global.d.ts {files}
```
...
pre-commit:
  ...
  jobs:
    - name: format frontend
      ...
      run: yarn oxfmt -c frontend/core/config/oxfmt.json {files}
      stage_fixed: true

    - name: lint frontend
      files: node scripts/lefthook-files.mjs frontend js jsx ts tsx
      run: yarn oxlint -c frontend/core/config/oxlint.json --fix --threads=1 {files}
      stage_fixed: true
...
- 执行 yarn lint-staged --debug，确认 workspace package.json 配置是否实际加载；
...
增加根目录 lefthook.yml。项目 .yarnrc.yml 已启用 enableScripts: true；使用官方 lefthook npm 包时，其 postinstall 会自动执行 lefthook install。根目录可以整体移除现有 postinstall: husky，避免再维护一份重复的安装命令，且不能让 Husky 和 Lefthook 同时接管 hooks。
...
CI：   workflow    -> yarn format:check / lint / type-check / test / build
```
...
4. 恢复 postinstall: husky，并执行 yarn 重新安装 Husky hooks；
...
2. 等价验证通过后，把全仓 yarn format:check 从默认 pre-commit 移出，改为 full-check 和 CI；
3. 最后再评估无写入 job 的并行化。
